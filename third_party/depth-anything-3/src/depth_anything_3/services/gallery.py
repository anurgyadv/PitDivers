...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box }
    ...
    /* True 3D fullscreen mode. The header stays over the model so the user
       can exit without relying exclusively on the Escape key. */
    ...
    .viewer.fullscreen-3d .viewer-body{
      display:block;
      height:100%;
      padding:0;
      overflow:hidden;
    }
    .viewer.fullscreen-3d .mv-box{
      width:100%;
      height:100%;
      aspect-ratio:auto;
      border:0;
    ...
    .viewer.fullscreen-3d .viewer-header{
      padding:6px;
      border-radius:12px;
  ...
  <div id="overlay" class="overlay" role="dialog" aria-modal="true" aria-label="3D Preview">
    <div class="viewer" id="viewer">
      <div class="viewer-header">
        ...
        <button id="fullscreenBtn" class="btn" title="Show only the 3D model using the full display">Full Screen</button>
...
function openViewer(i,{push=false}={}){currentScene=i;viewerTitle.textContent=i.title;mv.src=i.model;overlay.classList.add('show');viewer.classList.remove('fullscreen-3d');resGrid.hidden=true;toggleViewBtn.textContent='Resource View';toggleViewBtn.style.display='';fullscreenBtn.textContent='Full Screen';viewer.style.blockSize='min(82vh,var(--maxH))';buildResGrid(i,1);downloadBtn.onclick=()=>{const a=document.createElement('a');a.href=i.model;a.download=i.title+'.glb';a.click();};if(push){const u=new URL(location.href);if(!u.searchParams.get('group'))u.searchParams.set('group',currentGroup||'');u.searchParams.set('id',i.id);history.pushState(null,'',u);}}
...
function leaveFullscreen3D({restoreResources=true}={}){viewer.classList.remove('fullscreen-3d');fullscreenBtn.textContent='Full Screen';toggleViewBtn.style.display='';if(restoreResources){resGrid.hidden=!resourceViewBeforeFullscreen;toggleViewBtn.textContent=resGrid.hidden?'Resource View':'3D Only';}viewer.style.blockSize=resGrid.hidden?'min(82vh,var(--maxH))':'min(92vh,900px)';}
async function toggleFullscreen3D(){if(viewer.classList.contains('fullscreen-3d')){if(document.fullscreenElement){await document.exitFullscreen();}else{leaveFullscreen3D();}return;}resourceViewBeforeFullscreen=!resGrid.hidden;resGrid.hidden=true;viewer.classList.add('fullscreen-3d');viewer.style.blockSize='';fullscreenBtn.textContent='Exit Full Screen';toggleViewBtn.style.display='none';if(viewer.requestFullscreen){try{await viewer.requestFullscreen();}catch(e){console.warn('Browser fullscreen request failed; using viewport fullscreen fallback.',e);}}}
function closeViewer(){if(document.fullscreenElement){document.exitFullscreen().catch(()=>{});}leaveFullscreen3D({restoreResources:false});const hasId=!!qs().get('id');if(hasId&&history.length>1){history.back();return;}const u=new URL(location.href);u.searchParams.delete('id');history.replaceState(null,'',u);overlay.classList.remove('show');mv.src='';}
document.addEventListener('fullscreenchange',()=>{if(!document.fullscreenElement&&viewer.classList.contains('fullscreen-3d'))leaveFullscreen3D();});
overlay.onclick=e=>{if(e.target===overlay)closeViewer();};closeBtn.onclick=closeViewer;toggleViewBtn.onclick=toggleView;fullscreenBtn.onclick=toggleFullscreen3D;backBtn.onclick=()=>history.back();
